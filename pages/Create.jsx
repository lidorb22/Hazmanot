        ...
        <div className="h-screen flex flex-col font-sans overflow-hidden">
            ...
            <div className="h-full grid grid-cols-1 grid-rows-1 text-center">
                <div className="h-full w-full row-start-1 col-start-1 grid grid-rows-6 grid-cols-1">
                    {/* event selection */}
                    <motion.p
                    animate={isSelected ? {opacity: 0, scale: 0} : {opacity: 1, scale: 1}}
                    className="text-lg tracking-widest text-yellow-col font-bold col-start-1 row-start-2 self-center md:self-start">?מהו סוג האירוע</motion.p>
                    <form 
                    method="post" 
                    ...
                    className=" z-20 row-start-1 row-span-6 col-start-1 w-full h-full grid grid-rows-6">        
                        ...
                        <>
                            ...
                            <input name={qnaOBJ[invRison].names.f} onChange={invRison === 'Brit' ? e => setPlace(e.target.value) : e => setName(e.target.value)} type={qnaOBJ[invRison].type.f} className="w-2/3 bg-transparent border-b-2 border-black focus:outline-none md:w-2/6 text-center focus:text-center"/>
                            <label>{qnaOBJ[invRison].second}</label>
                            <input name={qnaOBJ[invRison].names.s} onChange={invRison === 'Brit' ? e => setTime(e.target.value) : e => setPlace(e.target.value)} type={qnaOBJ[invRison].type.s} className="w-2/3 bg-transparent border-b-2 border-black focus:outline-none md:w-2/6 text-center focus:text-center"/>
                            <label>{qnaOBJ[invRison].third}</label>
                            <input name={qnaOBJ[invRison].names.t} onChange={invRison === 'Brit' ? e => setDate(e.target.value) : e => setTime(e.target.value)} type={qnaOBJ[invRison].type.t} className="w-2/3 bg-transparent border-b-2 border-black focus:outline-none md:w-2/6 text-center focus:text-center"/>
                        </>
                        }
                        {qnaOBJ[invRison].forth != undefined &&
                            <>
                                <label>{qnaOBJ[invRison].forth}</label>
                                <input name={qnaOBJ[invRison].names.fo} onChange={invRison === 'Bday' ? e => setAge(e.target.value) : e => setDate(e.target.value)} type={qnaOBJ[invRison].type.fo} className="w-2/3 bg-transparent border-b-2 border-black focus:outline-none md:w-2/6 text-center focus:text-center"/>
                            </>
                        }
                        {qnaOBJ[invRison].five != undefined &&
                            <>
                                <label>{qnaOBJ[invRison].five}</label>
                                <input name={qnaOBJ[invRison].names.fiv} onChange={e => setDate(e.target.value)} type={qnaOBJ[invRison].type.fiv} className="w-2/3 bg-transparent border-b-2 border-black focus:outline-none md:w-2/6 text-center focus:text-center"/>
                            </>
                        }
                        </motion.div>
                        <button
                        type="submit" className="absolute bottom-12 right-0 text-lg font-bold w-full text-center h-10 bg-yellow-col border-t-2 border-b-2 border-black md:bottom-40">המשך</button>
                    </form>
                </div>
            </div>
