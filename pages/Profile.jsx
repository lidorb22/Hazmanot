...
import {inviteLogout, invitePending , inviteInfo , inviteFail, inviteResetList} from '../Slices/inviteSlice'
...
function Profile() {
    ...
    async function delHandeling(){
        dispatch(invitePending());
        try {
            await InvDelete({invId : invInfo[cardIndex]._id, userId : invInfo[cardIndex].inviter});
            let arr = [];
            invInfo.forEach(item => {
                arr.push(item);
            })
        ...
        <div className="w-full h-screen">
            ...
            <div className="w-full h-5/6 grid grid-rows-6 xl:grid-cols-2">
                <div className="row-start-1 col-start-1 justify-self-center self-center w-max text-center xl:col-start-2">
                    <p className="">שלום, <span className="font-bold">{fullName}</span></p>
                ...
                <motion.div 
                    ...
                    className="justify-self-center self-center w-full">
                        <div className="w-full flex flex-col items-center">
                            <p className="w-full bg-black text-white font-bold tracking-widest text-center">לא נמצאו ארועים</p>
                            <motion.div
                    ...
                    { 
                        invInfo.length > 0 && invInfo.map((item, index) => 
                            <div
                            key={item._id} 
                            className="font-bold w-full h-20 min-h-max">
                                <div 
                                id={item._id}
                ...
                <div className="flex flex-col row-start-6 col-start-1 w-5/6 h-12 bg-yellow-col shadow-try2 rounded-xl border-black self-center justify-self-center xl:col-span-2">
                    ...
                    className="self-center h-full w-1/2 flex justify-center">
                        <p className="pointer-events-none self-center text-center font-bold tracking-widest">יציאה</p>
                    </div>
                </div>
            </div>
            {invInfo.length > 0 && <motion.div 
            ...
            className="opacity-0 pointer-events-none bg-black bg-opacity-70 w-full h-full absolute top-0 right-0 z-50 flex flex-col items-center justify-center">
                ...
                className="bg-white w-5/6 h-4/6 rounded-2xl grid grid-rows-6">
                    <p className="justify-self-center mt-2 row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Bday' && 'יום ההולדת של'}</p>
                    <p className="justify-self-center mt-2 row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Hatona' && 'החתונה של' }</p>
                    <p className="justify-self-center mt-2 row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Hina' && 'החינה של'}</p>
                    <p className="justify-self-center mt-2 row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Bar' && 'הבר מצווה של'}</p>
                    <p className="justify-self-center mt-2 row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Bat' && 'הבת מצווה של'}</p>
                    <p className="justify-self-center self-center row-start-1 col-start-1 text-xl font-bold tracking-widest">{invInfo[cardIndex].invRison === 'Brit' && 'ברית'}</p>
                    <p className={`${invInfo[cardIndex].invRison === 'Bday' ? 'justify-self-end mr-16' : 'justify-self-center'} self-center mt-5 row-start-1 col-start-1 border-b-2 border-yellow-col`}>{invInfo[cardIndex].names}</p>
                    {invInfo[cardIndex].age !== '' &&<div className="flex flex-col items-center justify-center ml-16 justify-self-start self-center row-start-1 col-start-1 w-10 text-center h-10 border-yellow-col border-2 rounded-full mt-5">
                        <p className="">{invInfo[cardIndex].age}</p>
                    </div>}
                    <p className={`${invInfo[cardIndex].invRison === 'Brit' ? 'mb-3' : ''} text-xl  self-center justify-self-center row-start-1 row-span-2 col-start-1 tracking-widest`}>{invInfo[cardIndex].place}</p>
                    <p className="mr-5 justify-self-end self-center row-start-2 col-start-1">{invInfo[cardIndex].date}</p>
                    <p className="ml-5 justify-self-start self-center row-start-2 col-start-1">{invInfo[cardIndex].time}</p>
                    <div className="shadow-xl border-b-2 border-t-2 border-yellow-col justify-self-center self-start row-start-3 col-start-1 w-5/6 h-full flex flex-row items-center justify-center rounded-xl">
                        <p className="break-all p-4 text-center">localhost:3000/Invite/{invInfo[cardIndex]._id}</p>
                    </div>
                    ...
                    <div className="justify-self-center self-center row-start-4 row-span-3 col-start-1 w-full h-4/6 grid grid-cols-6">
                        <p className="font-bold border-b-2 border-yellow-col self-start justify-self-center row-start-1 col-start-1 ml-3 mt-6">מגיעים</p>
                        ...
                        <div className="shadow-xl border-b-2 border-t-2 border-yellow-col row-start-1 col-start-2 col-span-5 w-5/6 h-5/6 self-center justify-self-center rounded-xl flex flex-row items-center justify-center">list</div>
                    </div>
                    <div 
