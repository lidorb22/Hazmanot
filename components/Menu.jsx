...
import { MenuIcon, XIcon, UserIcon, SparklesIcon, HomeIcon } from '@heroicons/react/solid'
    ...
    return (
        ...
        ">
            ...
            <div className="w-full h-full bg-white row-start-1 row-span-2 rounded-b-full shadow-xl flex flex-col">
                <p className="text-center font-bold text-sm p-3">כדי להגיע למקומות אחרים באתר עלייך להתחבר או להרשם</p>
                <motion.p 
                initial={{y:0}}
                animate={{y:6}}
                transition={{
                    type: "Tween",duration: 0.5, repeat: Infinity, repeatType: "reverse",
                }}
                onClick={() => setAuthAcces(true)}
                className="text-center w-min font-bold text-sm bg-yellow-col px-5 cursor-pointer rounded-md self-center md:pointer-events-auto">כניסה</motion.p>
            </div>
            }
            {isAuth && 
            <div className="w-full h-full bg-yellow-col row-start-1 row-span-2 rounded-b-full shadow-2xl flex flex-col">
                ...
                onClick={(e) => NavigationHandler(e)}
                id="profile"
                className={`${Page === 'Invite' ? 'bg-transparent' :'bg-black'} flex flex-col w-full h-1/2 justify-center items-center `}>
                    <UserIcon className={`${Page === 'Invite' ? 'text-black' :'text-yellow-col'} pointer-events-none w-7`}/>
                    <p className={`${Page === 'Invite' ? 'text-black' :'text-yellow-col'} pointer-events-none`}>האזור האישי</p>
                </div>}
                {Page !== 'Invite' &&<div
                onClick={(e) => NavigationHandler(e)}
                id="invite"
            ...
            className="opacity-0 absolute top-0 w-full h-16 md:opacity-0 md:pointer-events-none">
                ...
                className="row-start-1 col-start-1 col-span-2 self-center justify-self-center bg-yellow-col w-1/2 h-5/6 text-center flex flex-col justify-center rounded-xl shadow-md">
                    <p className="pointer-events-none text-white font-bold tracking-widest">כניסה</p>
                </div>
                </motion.div>}
                {isAuth &&<motion.div 
                animate={isOpen ? "open" : "closed"}
